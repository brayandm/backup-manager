...
curl -o backup-manager https://raw.githubusercontent.com/brayandm/backup-manager/1.0.1/backup-manager.sh && \
...
echo 1.0.1 > VERSION && \
curl -o docker-compose.yml https://raw.githubusercontent.com/brayandm/backup-manager/1.0.1/docker-compose.yml
```
...
```bash
cd ~/.local/backup-manager/ && \
APP_PORT=<YOUR_CUSTOM_PORT> VERSION=1.0.1 docker compose up -d
```
